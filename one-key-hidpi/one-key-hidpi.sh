...
# 下载资源来自 https://github.com/lgs3137/MR_S1-macOS/tree/master/one-key-hidpi
function download(){
    echo '正在下载屏幕文件...'
    mkdir -p one-key-hidpi
    cd one-key-hidpi
    curl -fsSL https://raw.githubusercontent.com/lgs3137/MR_S1-macOS/master/one-key-hidpi/Icons.plist -O || networkWarn
    curl -fsSL https://raw.githubusercontent.com/lgs3137/MR_S1-macOS/master/one-key-hidpi/DisplayVendorID-dae/DisplayProductID-14d5 -O || networkWarn
    curl -fsSL https://raw.githubusercontent.com/lgs3137/MR_S1-macOS/master/one-key-hidpi/DisplayVendorID-dae/DisplayProductID-14d5.icns -O || networkWarn
    curl -fsSL https://raw.githubusercontent.com/lgs3137/MR_S1-macOS/master/one-key-hidpi/DisplayVendorID-dae/DisplayProductID-14d5.tiff -O || networkWarn
    echo '下载完成'
    echo
}
...
function copy() {
    ...
    sudo cp ./DisplayProductID-14d5 $DISPLAYPATH/DisplayVendorID-dae/
    sudo cp ./DisplayProductID-14d5.icns $DISPLAYPATH/DisplayVendorID-dae/
    sudo cp ./DisplayProductID-14d5.tiff $DISPLAYPATH/DisplayVendorID-dae/
    echo '拷贝完成'
    echo
}
...
function fixpermission() {
    ...
    sudo chown root:wheel $DISPLAYPATH/DisplayVendorID-dae/DisplayProductID-14d5
    sudo chown root:wheel $DISPLAYPATH/DisplayVendorID-dae/DisplayProductID-14d5.icns
    sudo chown root:wheel $DISPLAYPATH/DisplayVendorID-dae/DisplayProductID-14d5.tiff
    echo '修复完成'
    echo
}
